...
import { useQuizFlow } from "@/features/shared/hooks/useQuizFlow";
...
export default function QuizPage() {
  ...
  const { loading, submit } = useQuizFlow();
          ...
          <div className="flex flex-col gap-2 w-full">
            {current.options.map((option) => (
              <button
                ...
                className={`px-4 py-3 rounded-xl text-sm text-left border transition-all duration-300 backdrop-blur-md ${selected === option ? "bg-purple-500/20 border-purple-500/50 text-purple-200" : "bg-white/[0.02] border-white/[0.06] text-slate-400 hover:border-purple-500/30 hover:text-slate-200"}`}
              >
                {option}
              </button>
            ))}
