...
import { useRandomFlow } from "@/features/shared/hooks/useRandomFlow";
...
export default function RandomPage() {
  ...
  const { loading, submit } = useRandomFlow();
  const [selectedGenre, setSelectedGenre] = useState<string | null>(null);

  async function handleSortear() {
    await submit(selectedGenre);
  }
    ...
    <main className="relative flex flex-col items-center justify-center min-h-screen gap-8 p-8 bg-[#0a0a0c] overflow-hidden text-center">
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 max-w-lg">
        <div className="flex flex-col gap-2">
          <p className="text-[25px] mt-[-20px] tracking-[0.3em] uppercase text-indigo-300/60">
            Modo aleatório
          </p>
          ...
          <p className="text-slate-500 text-[20px] mt-[-15px] leading-relaxed">
            Sem quiz, sem perguntas. Escolha um gênero se quiser, ou deixa
            completamente no acaso.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 w-full">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {genres.map((genre) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs border transition-all duration-300 ${selectedGenre === genre ? "bg-indigo-500/20 border-indigo-500/50 text-indigo-300" : "bg-white/[0.02] border-white/[0.06] text-slate-500 hover:border-indigo-500/30 hover:text-slate-300"}`}
              >
                {genre}
              </button>
            ))}
          ...
          {selectedGenre && (
            <p className="text-[10px] text-indigo-400/60 text-center">
              Sorteando em{" "}
              <span className="text-indigo-300">{selectedGenre}</span> — clique
